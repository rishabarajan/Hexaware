...
public class InventoryManager {
    private SortedMap<Integer, Inventory> inventoryMap = new TreeMap<>(); // In-memory storage
    private DatabaseConnector dbConnector; // Database connection handler

    public InventoryManager() {
        this.dbConnector = new DatabaseConnector();
        loadInventoryFromDatabase(); // Sync database with in-memory store
    }
    ...
    private void loadInventoryFromDatabase() {
        ...
        try (Connection conn = dbConnector.openConnection();
            ...
            while (rs.next()) {
                ...
                // Create Product (Make sure to include stockQuantity)
                Product product = new Product(productID, productName, category, price, stockQuantity);

                // Create Inventory (Now includes inventoryID)
    ...
    // Add product (both in-memory & database)
    ...
    // Remove product from inventory & database
    ...
    private int generateInventoryID() {
        return (int) (Math.random() * 10000); // Generates a random inventory ID
    }
